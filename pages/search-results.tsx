...
import StatusBanner from "../components/Structure/StatusBanner"
...
const SearchResultsPage      = () => {
  ...
  const api = 'http://localhost:5000/api/products/search';
      ...
      if (query) {
        try {
          const response = await axios.get(api, {
            params: { search: query }, // Pass the search query to the backend
          ...
          const initialQuantities = response.data.reduce((acc: { [key: number]: number }, product: Product) => {
            acc[product.id] = 1; // Set product quantity to 1
            return acc;
          }, {});
  
          // Set the initial quantities in state
...
export default SearchResultsPage;  // Ensure correct default export
